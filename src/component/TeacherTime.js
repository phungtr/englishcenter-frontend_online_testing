...
const Scheduleteacher = ({ schedule }) => {
        ...
        <div className="schedule-griD">
          <div className="Time-slot">
            {timeSlots.map((time, index) => (
              <div key={index} className="time-slot">{time}____</div>
            ))}
          </div>

          <div className="calendar">
            {schedule.map((item, index) => {
              const startTimeValue = extractTime(item?.startTime);
              const endTimeValue = extractTime(item?.endTime);
              const top = timeToPosition(startTimeValue);
              const height = timeToPosition(endTimeValue) - top;
              const dayIndex = getDayIndex(item?.startTime);
              const eventClass = `event ${item.className.toLowerCase().replace(/\s+/g, "-")}`;
              return (
                <div
                  key={index}
                  className={eventClass}
                  style={{
                    backgroundColor: colorMapRef.current[item.className],
                ...
                >
                  <div className="event-content">
                    <div className="event-title" style={{ fontSize: "16px", fontWeight: "bold", textAlign: "center", margin: "20px 0 0 0" }}>
                      {item.className}
                    </div>
                    <div className="Des">
                      <div className="event-time" style={{ margin: "10px 0 0 0" }}>{startTimeValue} - {endTimeValue}</div>
                      <div className="event-teacher">{item.tcName}</div>
                    </div>
                  </div>
                </div>
              );
